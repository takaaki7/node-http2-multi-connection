...
var EndpointManager = protocol.EndpointManager;
...
function Server(options) {
  ...
  this._servers = [];
  this.endpointManager = new EndpointManager(this._log, 'round_robin');
...
Server.prototype.listen = function listen(portAndHosts) {
  var self = this;
  portAndHosts.forEach(function (portAndHost) {
    self._log.info({on: (portAndHost.host + ':' + portAndHost.port)},
      'Listening for incoming connections');
    var server = net.createServer(self._start.bind(self));
    server.on('close', self.emit.bind(self, 'close'));
    server.listen.apply(server, [portAndHost.port, portAndHost.host]);
...
Server.prototype.close = function close(callback) {
  ...
  this._servers.forEach(callback.bind(this));
};
...
OutgoingResponse.prototype.push = function push(options) {
  ...
  this._log.info({
    method: promise[':method'], scheme: promise[':scheme'],
...
function Agent(options) {
  ...
  if (!options.connections || options.connections.length === 0) {
    throw new Error('Connections must be set.');
  }
  this._settings = options.settings;
  this._log = (options.log || defaultLogger).child({component: 'http'});
  this.endpointManager = new EndpointManager(this._log, options.strategy || 'most_vacant');
  (options.connections || [{}]).forEach(function (connection) {
    connection.host = connection.hostname || connection.host || 'localhost';
    connection.port = connection.port || 443;
    connection.path = connection.path || '/';
  });
  var self = this;
  options.connections.forEach(function (connectionOption) {
    var endpoint = new Endpoint(self._log, 'CLIENT', self._settings);
    self._log.info('endpoint host:' + connectionOption.host +
      ',port:' + connectionOption.port + ',address:' + connectionOption.localAdress);
    endpoint.socket = net.connect({
      host: connectionOption.host,
      port: connectionOption.port,
      localAddress: connectionOption.localAddress
    });
    self.endpointManager.addEndpoint(endpoint);
...
Agent.prototype.request = function request(options, callback) {
  ...
  return this.endpointManager.createStream().then((stream)=> {
    request._start(stream, options);
