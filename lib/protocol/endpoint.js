...
Endpoint.prototype._initializeDataFlow = function _initializeDataFlow(role, settings, filters) {
  ...
  this.connection   = new Connection(this._log, firstStreamId, settings);

  pipeAndFilter(this.connection, this._compressor, filters.beforeCompression);
  ...
  pipeAndFilter(this._decompressor, this.connection, filters.afterDecompression);

  this.connection.on('ACKNOWLEDGED_SETTINGS_HEADER_TABLE_SIZE',
                      this._decompressor.setTableSizeLimit.bind(this._decompressor));
  this.connection.on('RECEIVING_SETTINGS_HEADER_TABLE_SIZE',
...
Endpoint.prototype._initializeManagement = function _initializeManagement() {
  this.connection.on('stream', this.emit.bind(this, 'stream'));
};
...
Endpoint.prototype._initializeErrorHandling = function _initializeErrorHandling() {
  ...
  this.connection.on('error', this._error.bind(this, 'connection'));

  this.connection.on('peerError', this.emit.bind(this, 'peerError'));
};
